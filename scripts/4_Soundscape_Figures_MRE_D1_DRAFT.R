...
#### Subset data frame to 3-day subsample to reflect analyzed dates ####
 hp_subset_all <- hp_og_all |>
  ...
  # long-term solution
  filter(Begin_Date_loc %in% seq(ymd("2020-02-09"), to = ymd("2020-05-03"),
                            by = "3 day") & Begin_Hour_loc %in% seq(3,23,1) | 
            Begin_Date_loc %in% seq(ymd("2020-02-10"), to = ymd("2020-05-03"),
                                    by = "3 day") & Begin_Hour_loc %in% seq(0,3,1))
...
Bal_diel_perc <- hp_subset_all |>
  ...
  pivot_longer(cols = c("freq"),
               values_to = "Perc_per_hour") |>
  filter(bal_pres > 0) |>
  mutate(Source = "Baleen whale")


# Dolphins
Dol_diel_perc <- hp_subset_all |>
  # group by Begin Hour to collapse into hours
  group_by(Begin_Hour_loc, dol_pres) |>
  # get total number of hours for Y and N groups per each hour per deployment
  summarize(n_hours = n())|>
  mutate(freq = n_hours/sum(n_hours)) |>
  ungroup() |>
  ...
  pivot_longer(cols = c("freq"),
               values_to = "Perc_per_hour") |>
  filter(dol_pres > 0) |>
  mutate(Source = "Delphinid spp.")

#### Merge all diel tables together for plotting ####

diel_perc_all <- Fish_ch_diel_perc |>
  merge(Fish_p_diel_perc, all = TRUE) |>
  merge(Ves_diel_perc, all = TRUE) |> # no vessels in subsampled dataset
  merge(Dol_diel_perc, all = TRUE) |>
...
ggsave("outputs/MRE_D1_diel_combined_1row.jpg", device = "jpeg",
...
# ggsave("outputs/MRE_D1_diel_vessel.jpg", device = "jpeg",
...
ggsave("outputs/MRE_D1_diel_fishchorus.jpg", device = "jpeg",
       width=10, height=10, units="in", dpi=600)
...
ggsave("outputs/MRE_D1_diel_fishpulse.jpg", device = "jpeg",
       width=10, height=10, units="in", dpi=600)



# Diel Polar Plot - Baleen (unk species)
...
#### Save individual baleen whale plot
ggsave("outputs/MRE_D1_diel_unkbaleen.jpg", device = "jpeg",
       width=10, height=10, units="in", dpi=600)




# Diel Polar Plot - Dolphins (unk species)
ggplot(data=Dol_diel_perc |> filter(dol_pres > 0), # only plot presence
       aes(x=Begin_Hour_loc, y = Perc_per_hour, fill = Perc_per_hour))+
  geom_bar(stat="identity")+
  scale_fill_viridis_c(begin = 0, end = 1, limits = c(0,1), breaks = c(0,0.25,0.5,0.75,1), option = "D")+
  ylab("N hours present /\nTotal deployment Hours") +
...
#### Save individual baleen whale plot
ggsave("outputs/MRE_D1_diel_dolphin.jpg", device = "jpeg",
       width=10, height=10, units="in", dpi=600)
...
# no vessels in subsampled dataset
# # Vessels
...
Bal_daily_perc <- hp_subset_all |>
  ...
  pivot_longer(cols = c("freq"),
               values_to = "Perc_per_day") |>
  filter(bal_pres > 0) |>
  mutate(Source = "Baleen")


# Baleen whales
Dol_daily_perc <- hp_subset_all |>
  # group by Begin Hour to collapse into hours
  group_by(Begin_Date_loc, dol_pres) |>
  # get total number of hours for Y and N groups per each hour per deployment
  summarize(n_hours = n())|>
  mutate(freq = n_hours/sum(n_hours)) |>
  ungroup() |>
...
daily_perc_all <- Fish_ch_daily_perc |>
  ...
  mutate(Source = factor(Source, levels = c("Fish pulse",
                                            ...
                                            "Baleen",
                                            "Vessel")),
         Lunar_ill = lunar.illumination(Begin_Date_loc, shift = 10)) #lunar illumination UTC+10
...
ggsave("outputs/MRE_D1_daily_combined_1row.jpg", device = "jpeg",
